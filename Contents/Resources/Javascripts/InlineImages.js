// Originally based on the Mochastyle JS

var originalAppendMessage = appendMessage;
var originalAppendNextMEsage = appendNextMessage;

// replace with ours that makes runs the parsing in the background 

appendMessage = function(html) {
	originalAppendMessage(html);
	setTimeout(function() { replaceLinksWithMedia(); }, 100);
}

appendNextMessage = function(html) {
	originalAppendNextMEsage(html);
	setTimeout(function() { replaceLinksWithMedia(); }, 100);
}

// run it initially
setTimeout(function() { replaceLinksWithMedia(); }, 200);


function replaceLinksWithMedia() {
